...
public class LumicodeWalkNodeProcessor extends LandPathNodeMaker {
    @Override
    ...
    private PathNode getNode(int x, int y, int z, int step, double floor, Direction dir) {
        ...
        if (atNode == PathNodeType.OPEN) {
            // account for node size
            Box collision = new Box(x - r + this.entityBlockXSize * 0.5D, y + 0.001D, z - r + this.entityBlockZSize * 0.5D, x + r + this.entityBlockXSize * 0.5D, y + this.entity.getHeight(), z + r + this.entityBlockZSize * 0.5D);
